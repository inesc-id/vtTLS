...
int main(int argc, char* argv[])
{
	...
	if (bytesToDownload >= 0) {
		...
		for (;;) {
                        ...
                        while (readResult == GNUTLS_E_AGAIN
                               || readResult == GNUTLS_E_INTERRUPTED);

			printf("Bytes received:%d\t", readResult);
			
			if (readResult == 0) {
				printf("- Peer has closed the TLS connection\n");
				goto end;
			} else if (readResult < 0 && gnutls_error_is_fatal(readResult) == 0) {
				fprintf(stderr, "*** Warning: %s\n", gnutls_strerror(readResult));
			...
			printf("Bytes downloaded:%d\t", bytesDownloaded);
			printf
	...
	gettimeofday(&end, NULL);
	diff = 1000 * (end.tv_sec - start.tv_sec) + (end.tv_usec - start.tv_usec) / 1000;
	printf("multiTLS with DTLS took %llu ms to transfer\n", diff);
